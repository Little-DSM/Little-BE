...
class PostService:
    ...
    def list_posts(
        ...
        if role is not None:
            stmt = stmt.where(MentoringPost.role == role)
    ...
    def apply_to_post(self, post_id: int, applicant: User) -> MentoringApplication:
        post = self.get_post(post_id)
        ...
        exists_stmt = select(MentoringApplication).where(
            ...
            MentoringApplication.mentor_id == applicant.id,
        )
        ...
        application = MentoringApplication(post_id=post_id, mentor_id=applicant.id)
    ...
    def create_or_update_review(
        ...
        mentee_id, mentor_id = self._resolve_participants(post, match)
        if user.id != mentee_id:
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
